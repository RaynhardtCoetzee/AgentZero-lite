...
function ArchDiagram() {
  ...
  return (
    <div ref={panelRef} className="relative flex-1 flex flex-col items-center justify-center px-10 py-20 z-10">
      <p className="font-mono text-[10px] text-muted-foreground/50 tracking-[0.12em] uppercase mb-8 self-start">
        architecture overview{' '}
        <span className="text-muted-foreground/20 text-[9px] tracking-[0.06em]">· hover to explore</span>
      </p>
      ...
      <svg viewBox="0 0 440 470" fill="none" xmlns="http://www.w3.org/2000/svg" className="w-full max-w-[440px]">
        ...
        <ellipse cx="182" cy="206" rx="130" ry="50" fill="var(--primary)" opacity="0.03" filter="url(#f-glow-lg)"/>
        ...
        <circle cx="295" cy="67"  r="3.5" fill="var(--primary)" opacity="0.6" filter="url(#f-glow-sm)"/>
        <circle cx="182" cy="186" r="3"   fill="var(--primary)" opacity="0.35"/>
        <circle r="3.5" fill="var(--primary)" opacity="0.9" filter="url(#f-glow-md)">
        ...
        <circle cx="282" cy="152" r="3.5" fill="var(--primary)" opacity="0.6" filter="url(#f-glow-sm)"/>
        <circle cx="182" cy="192" r="3"   fill="var(--primary)" opacity="0.35"/>
        <circle r="3.5" fill="var(--primary)" opacity="0.9" filter="url(#f-glow-md)">
        ...
        <circle cx="282" cy="283" r="3.5" fill="var(--primary)" opacity="0.6" filter="url(#f-glow-sm)"/>
        <circle cx="182" cy="218" r="3"   fill="var(--primary)" opacity="0.35"/>
        <circle r="3.5" fill="var(--primary)" opacity="0.9" filter="url(#f-glow-md)">
        ...
        <circle cx="276" cy="363" r="3.5" fill="var(--primary)" opacity="0.6" filter="url(#f-glow-sm)"/>
        <circle cx="182" cy="224" r="3"   fill="var(--primary)" opacity="0.35"/>
        <circle r="3.5" fill="var(--primary)" opacity="0.9" filter="url(#f-glow-md)">
        ...
        <g filter="url(#f-shadow)">
          ...
          <ellipse cx="330" cy="56" rx="56" ry="11" fill="url(#g-sat-top)" stroke="var(--primary)" strokeWidth="0.75"/>
        ...
        <g filter="url(#f-shadow)">
          ...
          <ellipse cx="330" cy="138" rx="62" ry="11" fill="url(#g-sat-top)" stroke="var(--primary)" strokeWidth="0.75"/>
        ...
        <g filter="url(#f-shadow)">
          ...
          <ellipse cx="330" cy="273" rx="62" ry="11" fill="url(#g-sat-top)" stroke="var(--primary)" strokeWidth="0.75"/>
        ...
        <g filter="url(#f-shadow)">
          ...
          <ellipse cx="330" cy="359" rx="56" ry="11" fill="url(#g-sat-top)" stroke="var(--primary)" strokeWidth="0.75"/>
        ...
        <g filter="url(#f-shadow)">
          ...
          <ellipse cx="182" cy="178" rx="132" ry="22" fill="none" stroke="var(--primary)" strokeWidth="0.75" strokeOpacity="0.12"/>
        </g>
        ...
        <text x="182" y="174" fontFamily="ui-monospace,monospace" fontSize="7.5" fontWeight="400" fill="var(--primary)" fillOpacity="0.4" textAnchor="middle" letterSpacing="0.14em">v1.0.0</text>
      ...
      {activeTip && tip && (
        <div
          className="absolute pointer-events-none bg-card border border-white/[11%] rounded-[5px] px-3 py-2 font-mono text-[11px] text-foreground z-10 max-w-[200px] leading-relaxed shadow-[0_4px_20px_rgba(0,0,0,0.5)]"
          style={{ left: Math.min(tip.x - 10, 230) + 'px', top: tip.y - 90 + 'px' }}
        >
          <span className="block font-semibold text-primary text-[10px] tracking-[0.08em] uppercase mb-1">
            {activeTip.title}
          </span>
          {activeTip.body}
        </div>
...
export function HeroSection() {
  return (
    <section
      className="min-h-dvh grid grid-cols-1 md:grid-cols-[1fr_520px] pt-[52px]"
    >
      {/* Left column */}
      <div
        className="relative flex flex-col justify-center px-5 sm:px-8 md:px-10 py-14 sm:py-20 md:border-r md:border-white/[7%]"
      ...
      >
        {/* Status tag */}
        <div
          className="inline-flex items-center gap-[7px] font-mono text-[11px] text-muted-foreground/50 tracking-[0.06em] mb-9 px-[11px] py-[5px] border border-white/[11%] rounded-[3px] bg-background w-fit"
          style={{ animation: 'fup 0.5s ease 0.1s both' }}
        >
          <span className="w-[5px] h-[5px] rounded-full bg-primary" style={{ animation: 'ppulse 2.4s ease-in-out infinite' }}/>
          v1.0 · founding member window open
        </div>

        {/* Headline */}
        <h1
          className="font-mono font-bold leading-[0.97] tracking-[-0.04em] text-foreground mb-7"
          style={{ fontSize: 'clamp(34px, 4.8vw, 64px)', animation: 'fup 0.55s ease 0.18s both' }}
        >
          THE HARDENED<br/>
          BOILERPLATE<br/>
          FOR <span className="text-primary">AGENTS</span><span className="text-muted-foreground/50">.</span>
        </h1>

        {/* Body copy */}
        <p
          className="text-[15px] font-light text-muted-foreground leading-[1.7] max-w-[460px] mb-10"
          style={{ animation: 'fup 0.55s ease 0.26s both' }}
        >
          Skip <strong className="text-foreground font-normal">40–80 hours</strong> of architecture work. Deploy a billable AI product in one day — auth, agents, RAG, billing, credit guards, all wired together and ready to ship.
        </p>

        {/* Stack line */}
        <p
          className="font-mono text-[11px] text-muted-foreground/50 tracking-[0.03em] mb-9 -mt-7"
          style={{ animation: 'fup 0.55s ease 0.30s both' }}
        >
          <strong className="text-muted-foreground font-normal">Next.js 16 · AI SDK 6 · Supabase · Auth.js v5 · Lemon Squeezy · pgvector</strong>
        </p>

        {/* CTAs */}
        <div
          className="flex flex-col sm:flex-row sm:items-center gap-2.5 mb-[52px]"
          style={{ animation: 'fup 0.55s ease 0.34s both' }}
        >
          <a
            href="#pricing"
            className="inline-flex items-center justify-center gap-2 bg-primary text-primary-foreground font-mono text-[13px] font-bold px-6 py-3 sm:py-[11px] rounded-[5px] transition-[opacity,transform] duration-150 hover:opacity-[0.88] hover:-translate-y-px min-h-[44px] w-full sm:w-auto"
          >
          ...
          <a
            href="#manifest"
            className="inline-flex items-center justify-center gap-2 bg-transparent text-muted-foreground font-mono text-[13px] px-[18px] py-3 sm:py-[11px] rounded-[5px] border border-white/[11%] transition-[color,border-color,background] duration-150 hover:text-foreground hover:border-white/[18%] hover:bg-card min-h-[44px] w-full sm:w-auto"
          >
        ...
        {/* Price note */}
        <p
          className="font-mono text-[11px] text-muted-foreground/50 -mt-9 mb-9 tracking-[0.02em]"
          style={{ animation: 'fup 0.55s ease 0.38s both' }}
        >
          Founding member price — <span className="line-through text-muted-foreground/20">$249</span> $129. One-time. No subscription.
        </p>
        ...
        <div className="flex gap-2 flex-wrap" style={{ animation: 'fup 0.55s ease 0.42s both' }}>
          {PROOF_PILLS.map((label) => (
            <span
              key={label}
              className="inline-flex items-center gap-[6px] font-mono text-[11px] text-muted-foreground/50 px-[10px] py-1 border border-white/[7%] rounded-full bg-background tracking-[0.02em]"
            >
              <svg width="10" height="10" viewBox="0 0 24 24" fill="none" stroke="var(--primary)" strokeWidth="2.5"><polyline points="20 6 9 17 4 12"/></svg>
              {label}
            </span>
          ))}
        </div>
        ...
        {/* Terminal */}
        <div
          className="mt-11 max-w-[500px] bg-background border border-white/[11%] rounded-lg overflow-hidden relative z-10"
          style={{ animation: 'fup 0.55s ease 0.5s both' }}
        >
          <div className="flex items-center gap-1.5 px-3.5 py-[9px] bg-card border-b border-white/[7%]">
            <div className="w-2.5 h-2.5 rounded-full bg-[#ff5f57]"/>
            <div className="w-2.5 h-2.5 rounded-full bg-[#febc2e]"/>
            <div className="w-2.5 h-2.5 rounded-full bg-[#28c840]"/>
            <span className="ml-1.5 font-mono text-[11px] text-muted-foreground/50">bash — ~/projects</span>
          </div>
          <div className="px-[18px] py-4 font-mono text-[12.5px] leading-[2] flex flex-col">
            <span>
              <span className="text-muted-foreground/50">$ </span>
              <span className="text-primary">git</span>
              <span className="text-foreground"> clone https://github.com/agentzero/boilerplate</span>
              <span className="inline-block w-[7px] h-[13px] bg-primary align-middle ml-0.5" style={{ animation: 'blink 1s step-end infinite' }}/>
            </span>
            <span className="text-muted-foreground/50 opacity-0" style={{ animation: 'fup 0.35s ease 1.3s both' }}>  Cloning into &apos;boilerplate&apos;...</span>
            <span className="text-muted-foreground/50 opacity-0" style={{ animation: 'fup 0.35s ease 1.8s both' }}>  remote: Enumerating objects: 1,842 done.</span>
            <span className="text-muted-foreground/50 opacity-0" style={{ animation: 'fup 0.35s ease 2.3s both' }}>  Receiving objects: 100% (1842/1842), 4.2 MB</span>
            <span className="text-muted-foreground/50 opacity-0" style={{ animation: 'fup 0.35s ease 2.8s both' }}>
              <span className="text-muted-foreground/50">$ </span>
              <span className="text-primary">cp</span>
              <span className="text-foreground"> .env.example .env.local</span>
            </span>
            <span className="text-primary opacity-0" style={{ animation: 'fup 0.35s ease 3.2s both' }}>  ✓ ready. fill in your keys and deploy.</span>
          </div>
        </div>
      </div>
      ...
      <div
        className="hidden md:flex flex-col bg-background relative overflow-hidden"
