...
export function ManifestSection() {
  ...
  return (
    <section className="py-[100px] px-10 bg-background" id="manifest">
      <div className="max-w-[1100px] mx-auto">
        <div className="flex items-center gap-2.5 font-mono text-[10px] text-muted-foreground/50 tracking-[0.12em] uppercase mb-3.5">
        ...
        <div className="mt-[52px] border border-white/[7%] rounded-[10px] overflow-hidden grid grid-cols-1 md:grid-cols-[1fr_1fr_1fr] bg-white/[7%] gap-px">
          {/* Tree */}
          <div className="bg-background p-7 font-mono text-[13px] md:col-span-1">
            <div className="flex items-center gap-2 font-mono text-[10px] text-muted-foreground/50 tracking-[0.1em] uppercase pb-3.5 mb-2.5 border-b border-white/[7%]">
              <span className="w-3 h-3 border border-white/[11%] rounded-[3px] bg-card"/>
              project structure
            </div>
            {TREE.map((item) => (
              <div
                ...
                className={`flex items-center py-[5px] px-2 rounded cursor-pointer transition-colors gap-0 ${
                  selected === item.id
                    ? 'bg-primary/[8%] border border-primary/[28%]'
                    : 'hover:bg-card border border-transparent'
                }`}
              >
                <span className="text-muted-foreground/20 whitespace-pre text-[12px]">{item.prefix}</span>
                <span className={`font-medium ${item.isDir ? 'text-primary/80' : selected === item.id ? 'text-primary' : 'text-foreground'}`}>
                  {item.label}
                </span>
                {item.desc && (
                  <>
                    <span className="text-muted-foreground/20 mx-2 text-[11px]">—</span>
                    <span className="text-muted-foreground/50 text-[11px]">{item.desc}</span>
                  </>
                )}
              </div>
            ))}
          ...
          {/* Detail */}
          <div className="bg-background p-7 flex flex-col md:col-span-2">
            <div className="flex items-center gap-2 font-mono text-[10px] text-muted-foreground/50 tracking-[0.1em] uppercase pb-3.5 mb-[18px] border-b border-white/[7%]">
              <span className="w-3 h-3 border border-white/[11%] rounded-[3px] bg-card"/>
              module detail
            </div>
            <div className="flex items-center gap-1.5 font-mono text-[11px] text-primary mb-2.5">
              <span className="text-muted-foreground/50">→</span>
              {detail.path}
            </div>
            <div className="font-mono text-[18px] font-semibold text-foreground mb-3 tracking-[-0.025em] leading-[1.2]">
              {detail.title}
            </div>
            <p className="text-[13px] text-muted-foreground leading-[1.75] flex-1 mb-5 font-light">
              {detail.body}
            </p>
            <div className="flex flex-wrap gap-[5px] mt-auto">
              {detail.tags.map((tag) => (
                <span
                  key={tag}
                  className="font-mono text-[10px] text-muted-foreground/50 bg-card border border-white/[11%] px-[9px] py-[3px] rounded-[3px] tracking-[0.03em] hover:border-primary/[28%] hover:text-primary/80 transition-colors cursor-default"
                >
                  {tag}
                </span>
              ))}
